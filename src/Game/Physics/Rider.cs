...
namespace linerider.Game
{
    ...
    public struct Rider
    {
        ...
        {
            if (Settings.FixedCam)
                return (new Vector2d((Settings.HorizontalTracking ? Body[4].Location.X : Settings.XFixed), (Settings.VerticalTracking ? Body[4].Location.Y : Settings.YFixed)));
            if (Settings.VerticalTracking)
                return (new Vector2d(0, Body[4].Location.Y));
            if (Settings.HorizontalTracking)
                return (new Vector2d(Body[4].Location.X, 0));
            if (Settings.OffsledVar)
                return Body[Settings.PointVar].Location;
            if (Crashed)
                return (Settings.OffsledSledCam ? ((Body[2].Location + Body[1].Location + Body[3].Location + Body[0].Location) / 4) : Body[4].Location);
            ...
            int frameid = 0)
        {
            ...
            if (stepscarf)
            {
                ...
                if (Settings.multiScarfAmount * Settings.multiScarfSegments > Settings.ScarfSegments) { Settings.multiScarfAmount = 1; } //if too big set to zero
                ...
                if (Settings.multiScarfAmount > 1) //If using dual scarf
                {
                    ...
                    for (int i = 0; i < Settings.multiScarfAmount; i++) //For each scarf
                    {
